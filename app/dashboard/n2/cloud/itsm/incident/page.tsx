...
export default function CloudITSMIncidentPage() {
  ...
  const { backlogs, saveBacklog, deleteBacklog, loadBacklog, exportBacklog } =
    useBacklogs("cloud-itsm-incident");
  ...
  const filteredTickets = useMemo(() => {
    ...
    try {
      ...
      return tickets.filter((ticket) => {
        // Only include INC tickets
        if (!ticket.id.startsWith("INC") || ticket.type !== "INC") return false;
  ...
  const handleExport = useCallback(() => {
    ...
    try {
      ...
      const fileName = `cloud-itsm-incident-${formatDate(new Date(), "yyyy-MM-dd-HH-mm")}.xlsx`;
    ...
    try {
      ...
      if (backlog) {
        const restored = backlog.content
          .filter((ticket: ITSMTicket) => ticket.id.startsWith("INC") && ticket.type === "INC")
        ...
        setSelectedStatus("");
        setSelectedPriority("");
        toast.success(`Loaded incident backlog "${backlog.title}"`);
      }
    } catch (error) {
  ...
  const handleFileProcessed = useCallback((newTickets: ITSMTicket[]) => {
    const validTickets = newTickets.filter(
      (ticket) => ticket.id.startsWith("INC") && ticket.type === "INC"
    );
    setTickets(validTickets);
    toast.success(`Loaded ${validTickets.length} incidents`);
  }, []);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
  title="Incident Management (Cloud)"
  description="Manage cloud incidents (INC) and track their resolution."
>
      ...
      {tickets.length === 0 ? (
        ...
        <Tabs defaultValue="overview" className="space-y-4">
          ...
          <TabsContent value="tickets">
            <ContentCard title="Incident Management">
              <div className="space-y-4">
                <FilterControls
                  ...
                  selectedType="_all" // Type filter hidden for INC-only page
