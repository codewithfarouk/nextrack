...
interface TicketTableProps {
  filteredTickets: ITSMTicket[];
   isLoading?: boolean; // ✅ Ajouté ici (optionnel si tu veux la rendre non obligatoire)
}
...
export const TicketTable: React.FC<TicketTableProps> = ({ filteredTickets }) => {
  ...
  const isTicketOverdue = useCallback((ticket: ITSMTicket): boolean => {
    ...
    const isClosedStatus = ["résolu", "clos", "fermé", "closed", "clot"].includes(normalizedStatus);
  ...
  const getPriorityNumber = useCallback((priority: string): number => {
    return parseInt(priority.replace("P", ""), 10) || 4; // Unknown priorities sort last
  }, []);
  ...
  const getSummaryStats = useMemo(() => {
    ...
    const critical = sortedTickets.filter((t) => t.priority === "P1").length;
                    ...
                    >
                      ...
                      >
                        {ticket.lastUpdatedAt instanceof Date && !isNaN(ticket.lastUpdatedAt.getTime())
                          ? formatDate(ticket.lastUpdatedAt, "dd/MM/yyyy HH:mm")
                          : "-"}
                      </TableCell>
