...
export const OverviewStats: React.FC<OverviewStatsProps> = ({
  ...
  const stats = useMemo(() => {
    ...
    // Change requests (type: "CRQ")
    const changeRequests = filteredTickets.filter((t) => t.type === "CRQ").length;

    // Incidents (type: "INC")
    const incidents = filteredTickets.filter((t) => t.type === "INC").length;

    // High priority tickets (priority: "P1")
    const highPriority = filteredTickets.filter((t) => t.priority === "P1").length;
